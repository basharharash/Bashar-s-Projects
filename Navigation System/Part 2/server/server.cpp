...
int main() {
  ...
  // array of buffers type chars
  char packet[1024] = {0};
  int C_flag = false;  // for debug
  // build the graph
  ...
  while (true) {
    ...
    Point sPoint, ePoint;
    // ask for requests
    request >> Slat >> Slon >> Elat >> Elon;

    // cast to long long
    sPoint.lat = static_cast<long long>(Slat*100000.0);
    sPoint.lon = static_cast<long long>(Slon*100000.0);
    ePoint.lat = static_cast<long long>(Elat*100000.0);
    ePoint.lon = static_cast<long long>(Elon*100000.0);
    ...
    if (tree.find(end) == tree.end()) {
      ...
      for (int v : path) {
        // cast back to double
        Slat = static_cast<double>(points[v].lat/100000.0);
        Slon = static_cast<double>(points[v].lon/100000.0);
        // cast to string
        string S_lat = to_string(Slat);
        string S_lon = to_string(Slon);
        // get rid of trailing 0's
        S_lat.pop_back();
        S_lon.pop_back();
        // form the waypoint
        string target = S_lat + ' ' + S_lon + '\n';
        // send the waypoint
        write(out, target.c_str(), target.length());
      }
    }
    // send after the last waypoint
    write(out, "E\n", 2);
  }
  // we are done, close and unlink the pipes
  close(in);
  close(out);
  unlink(inpipe);
  unlink(outpipe);
